...
class Angelleye_Gravity_Braintree_ACH_Field extends GF_Field {
	...
	function overrideSubmitButton( $button, $form ) {
		return "<button class='button gform_button custom_ach_form_submit_btn' id='gform_submit_button_{$form['id']}' 
onclick='if(window[\"gf_submitting_{$form['id']}\"]){return false;}  window[\"gf_submitting_{$form['id']}\"]=true;'
onkeypress='if( event.keyCode == 13 ){ if(window[\"gf_submitting_{$form['id']}\"]){return false;} window[\"gf_submitting_{$form['id']}\"]=true;  jQuery(\"#gform_{$form['id']}\").trigger(\"submit\",[true]); }'
><span>{$form['button']['text']}</span></button>";
	}
